    ...
    .directive('cacheSrc', function($ionicPlatform, $window, $interval, $timeout, $compile, $cacheSrc, $cordovaFileTransfer, $localStorage,$cordovaFile) {
      return {
        ...
        link: function(scope, element, attrs) {
          ...
          if ($window.cordova) {
            var getCacheDir = function(convert) {
              ...
              if (window.device)
                if (window.cordova.file) {
                  switch (device.platform) {
                    case 'iOS':
                      return convert ? window.Ionic.WebView.convertFileSrc($window.cordova.file.documentsDirectory) : $window.cordova.file.documentsDirectory;
                    case 'Android':
                      return convert ? window.Ionic.WebView.convertFileSrc($window.cordova.file.dataDirectory) : $window.cordova.file.dataDirectory;
                    case 'windows':
                      return convert ? window.Ionic.WebView.convertFileSrc($window.cordova.file.dataDirectory) : $window.cordova.file.dataDirectory;
                  }
                } else
            ...
            function fetchRemoteWithoutLoading() {
              ...
              var fileName = id() + ext.split('?')[0];
              $cordovaFileTransfer
                .download(attrs.cacheSrc, getCacheDir(false) + fileName, {
